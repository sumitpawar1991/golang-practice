...
func (h *Handler) CreateNote(c *gin.Context) {
	// per req object from gin
	//c.req c.param

	var req CreateNoteRequest

	if err := c.ShouldBindJSON(&req); err != nil {
		c.JSON(http.StatusBadRequest, gin.H{"error": "Invaild Json"})
		return
	}
	...
	if err != nil {
		c.JSON(http.StatusInternalServerError, gin.H{"error": "Failed to create note here!!"})
	...
	if err != nil {
		if errors.Is(err, mongo.ErrNoDocuments) {
			c.JSON(http.StatusNotFound, gin.H{
				"error": "Note not foind for that given id",
			})
			return
		}
